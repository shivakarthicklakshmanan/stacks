...
class AgentPanel {
        ...
        this.panel.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.type) {
                ...
                case "applyToActiveFile":
                    await this.applyToActiveFile(msg.code, msg.fullResponse);
                    break;
                case "newSession":
                    this.newSession();
                    break;
    ...
    async applyToActiveFile(suggestedCode, fullResponse) {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            this.postToWebview({ type: 'applyError', reason: 'No file is currently open in the editor. Open the file you want to update and try again.' });
            return;
        }
        const fileName = editor.document.fileName.split(/[\\/]/).pop() || 'file';
        const currentContent = editor.document.getText();
        if (!currentContent.trim()) {
            // File is empty — just write the suggested code directly
            await editor.edit((eb) => {
                eb.replace(new vscode.Range(0, 0, editor.document.lineCount, 0), suggestedCode);
            });
            this.postToWebview({ type: 'applyDone' });
            return;
        }
        // Build a merge prompt — ask Llama to apply the suggestion to the full file
        const mergePrompt = `You are a code editor assistant. Your job is to apply a suggested change to an existing file.
...
INSTRUCTIONS:
- Apply the suggested change to the existing file intelligently
- If the suggestion is a new function/class, add it in the appropriate place
- If the suggestion fixes a bug, replace only the affected lines
- If the suggestion is a complete rewrite, use it entirely
- Preserve all existing code that is NOT being changed
- Return ONLY the complete updated file content with NO explanation, NO markdown, NO code fences
- Return raw code only, starting from the first line of the file`;
        const messages = [
            { role: 'system', content: 'You are a precise code editor. Return only the complete updated file content with no explanation or markdown formatting.' },
            { role: 'user', content: mergePrompt },
        ];
        // Use Llama for the merge (structured output model)
        const result = await (0, llmRegistry_1.callWithFallback)('Llama-3.3-70B', messages, this.getEnabledModels());
        if (!result.text) {
            this.postToWebview({ type: 'applyError', reason: `Llama could not process the merge: ${result.error}` });
            return;
        }
        // Strip any accidental code fences Llama may have added
        let mergedContent = result.text.trim();
        mergedContent = mergedContent.replace(/^```[\w]*\n?/m, '').replace(/\n?```$/m, '').trim();
        // Apply to the editor — replace entire file content
        const fullRange = new vscode.Range(
            editor.document.positionAt(0),
            editor.document.positionAt(editor.document.getText().length)
        );
        await editor.edit((eb) => {
            eb.replace(fullRange, mergedContent);
        });
        // Save the file automatically
        await editor.document.save();
        this.postToWebview({ type: 'applyDone' });
        vscode.window.showInformationMessage(`TechMind applied changes to ${fileName} and saved.`);
    }
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .btn-apply:hover { background: var(--vscode-button-hoverBackground) !important; }
  /* ── Applying spinner ── */
  #applyingBanner {
    font-size: 11px; opacity: 0.7;
    padding: 3px 14px; display: none;
    font-style: italic; color: var(--tm-accent);
  }
</style>
</head>
<body>
  ...
  <div id="applyingBanner">Applying code to active file via Llama...</div>
  <div id="chat"></div>
  <div id="inputBar">
    <button id="attachBtn" title="Attach file (text, image, PDF)">+ File</button>
...
<script>
  ...
  const applyingBanner = document.getElementById('applyingBanner');
  ...
  function addMessage(role, text, meta) {
    ...
    if (role === 'assistant') {
      ...
      if (blocks.length > 0) {
        ...
        insertBtn.title = 'Insert code at current cursor position in active editor';
        ...
        applyBtn.title = 'Let Llama merge this suggestion into the currently open file intelligently';
        applyBtn.onclick = () => {
          applyingBanner.style.display = 'block';
          vscode.postMessage({ type: 'applyToActiveFile', code: blocks.join('\n\n'), fullResponse: text });
        };

        const saveBtn = document.createElement('button');
        saveBtn.textContent = '💾 Save as .py';
        saveBtn.title = 'Save code as a new Python file';
  ...
  window.addEventListener('message', (event) => {
    ...
    switch (msg.type) {
      ...
      case 'filesUpdated':
        ...
        break;
      case 'applyDone':
        applyingBanner.style.display = 'none';
        addMessage('assistant', '✅ **Applied successfully** — the active file has been updated with the suggested changes.', null);
        break;
      case 'applyError':
        applyingBanner.style.display = 'none';
        addMessage('assistant', '❌ Could not apply: ' + msg.reason, null);
        break;
      case 'contextCleared':
